...
class HUD(object):
    ...
    def tick(self, world, clock):
        ...
        self._info_text = [
            'Server:  % 20.0f FPS' % self.server_fps,
            'Client:  % 20.0f FPS' % clock.get_fps(),
            '',
...
class CameraManager(object):
    ...
    def _parse_image(weak_self, image):
        ...
        if self.recording:
            ...
            obstacles = self.detect_obstacles(image_data)
            print('Obstacles', obstacles)
        
            cascade_file_path = 'car.xml'
            cascade_classifier = cv2.CascadeClassifier(cascade_file_path)
            
            gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
            obstacles_rects = cascade_classifier.detectMultiScale(gray, scaleFactor=1.1,
                                                                    minNeighbors=5, minSize=(30, 30))
            #obstacles = [Obstacle(x, y, w, h) for (x, y, w, h) in obstacles_rects]
            obstacles = obstacles_rects.tolist()
            print("Obstacles: ", obstacles)
...
class VehiclePIDController():
    ...
    def __init__(self, vehicle, args_lateral, args_longitudinal, max_throttle=1.0, max_brake=0.3, max_steering=0.5):
        """
        Constructor method.
    ...
    def avoid_obstacles(obstacles):
        avoidance_control = carla.VehicleControl()

        if len(obstacles) > 0:
            # Select the closest obstacle
            closest_obstacle = min(obstacles, key=lambda ob: ob[0])

            # Calculate desired steer angle based on obstacle position
            obstacle_x = closest_obstacle[0]
            steer_angle = (obstacle_x - 320) / 320 * 0.4  # Adjust steering sensitivity as needed

            # Apply steer angle for obstacle avoidance
            avoidance_control.steer = steer_angle
            avoidance_control.throttle = 0.5  # Adjust throttle as needed
            avoidance_control.brake = 0.0
...
class PIDLongitudinalController():
    ...
    def run_step(self, target_speed, obstacle_detected=False, debug=False):
        """
        ...
        """
        current_speed = get_speed(self._vehicle) / 1.5
        #target_speed *= 0.5
        target_speed = target_speed / 1.5

        if obstacle_detected == True:
            # Apply obstacle avoidance logic
            target_speed *= 0.7  # Reduce target speed when obstacle detected
...
class PIDLateralController():
    ...
    def run_step(self, waypoint, obstacle_detected=False):
        """
        ...
        """
        def get_obstacle_actor():
            # Retrieve the obstacle actor based on its attributes or criteria
            # Here's an example of finding the first obstacle actor with a specific role name
            obstacle_actors = self.world.get_actors().filter('vehicle.*')  # Filter obstacle actors based on their type
            obstacle_actors.set_attribute('role_name', 'obstacle')
            for actor in obstacle_actors:
                if actor.attributes.get('role_name') == 'obstacle':
                    return actor

            return None  # Return None if no obstacle actor is found
        
        def get_obstacle_position():
            # Assuming you have access to the obstacle actor or its transform
            obstacle_actor = get_obstacle_actor()

            # Get the position of the obstacle actor
            obstacle_position = obstacle_actor.get_location()

            return obstacle_position
        
        def get_obstacle_size():
            # Assuming you have access to the obstacle actor or its bounding box
            obstacle_actor = get_obstacle_actor()

            # Get the bounding box of the obstacle actor
            obstacle_bounding_box = obstacle_actor.bounding_box

            # Calculate the size of the obstacle (width in this case)
            obstacle_size = obstacle_bounding_box.extent.x * 2.0

            return obstacle_size

        
        def adjust_waypoint_for_obstacle(waypoint):
            # Obtain the obstacle position and size from the obstacle detection system
            obstacle_position = get_obstacle_position()
            obstacle_size = get_obstacle_size()

            # Calculate the distance between the waypoint and the obstacle
            waypoint_position = waypoint.transform.location
            distance = math.sqrt((waypoint_position.x - obstacle_position.x) ** 2 + (waypoint_position.y - obstacle_position.y) ** 2)

            # Adjust the waypoint based on the obstacle size and distance
            adjusted_waypoint = waypoint

            if distance < obstacle_size:
                # Calculate the desired offset from the obstacle based on its size
                offset = obstacle_size + 1.0  # Add a safety margin

                # Calculate the direction vector from the obstacle to the waypoint
                direction_vector = carla.Vector3D(waypoint_position.x - obstacle_position.x, waypoint_position.y - obstacle_position.y, 0.0)
                direction_vector = direction_vector / np.linalg.norm(direction_vector)  # Normalize the direction vector

                # Calculate the adjusted waypoint position
                adjusted_waypoint_position = carla.Location(
                    obstacle_position.x + offset * direction_vector.x,
                    obstacle_position.y + offset * direction_vector.y,
                    waypoint_position.z
                )

                # Create a new waypoint with the adjusted position
                adjusted_waypoint = carla.Waypoint(adjusted_waypoint_position, waypoint.transform.rotation)

            return adjusted_waypoint

        if obstacle_detected:
            # Apply obstacle avoidance logic
            waypoint = adjust_waypoint_for_obstacle(waypoint)  # Implement your own waypoint adjustment logic
...
class LocalPlanner(object):
    ...
    def __init__(self, agent, min_distance):
        """
    ...
    def get_distance_to_nearest_obstacle(self):
        ...
        #self.camera_manager = CameraManager(self._parent, self.hud, self.gamma_correction, self.obstacle_detection)
        #self.camera_manager.sensor.listen(self.camera_manager._process_image_callback)
    '''   
    def get_distance_to_nearest_obstacle(self):
        """
        Calculate the distance to the nearest obstacle.

        :return: distance to the nearest obstacle (float) or None if no obstacle detected
        """
        obstacles = self.obstacle_detection.detect_obstacles()  # Perform obstacle detection
        if obstacles:
            distances = [self.calculate_distance(obstacle) for obstacle in obstacles]  # Calculate distances to obstacles
            return min(distances)  # Return the minimum distance
        else:
            return None  # No obstacle detected, return None
        
    def calculate_distance(self, obstacle):
        """
        Calculate the distance to the given obstacle using the Euclidean distance formula.

        :param obstacle: obstacle object
        :return: distance to the obstacle (float)
        """
        ego_vehicle_loc = self._vehicle.get_location()
        obstacle_loc = obstacle.get_location()

        # Calculate the Euclidean distance between ego vehicle and obstacle
        distance = math.sqrt((obstacle_loc.x - ego_vehicle_loc.x) ** 2 + (obstacle_loc.y - ego_vehicle_loc.y) ** 2)
...
class BehaviorAgent(Agent):
    ...
    def __init__(self, vehicle, ignore_traffic_light=False, behavior='normal', min_distance=5.0):
        """
        Constructor method.
        ...
        self._local_planner = LocalPlanner(self, min_distance)
        self._grp = None
        ...
        '''
         ...
         #   control = self._local_planner.run_step(
          #      target_speed=min(self.behavior.max_speed, self.speed_limit - self.behavior.speed_lim_dist), debug=debug)
            ...
            # Vehicle detected a red light, apply a stop control
         #   control.throttle = 0.0
          #  control.brake = 1.0
        #else:
            # Vehicle is not at a red light, continue with the local planner
         #   control = self._local_planner.run_step(
...
def game_loop(args):
    ...
    try:
        ...
        #selected_world = client.load_world("Town05")  # KK Changes world to Town04
